...
export default function FollowUpReminders({ followUps }: FollowUpRemindersProps) {
    ...
    const sortedFollowUps = [...followUps].sort((a, b) => {
        const dateA = a.nextFollowUp ? new Date(a.nextFollowUp).getTime() : Infinity;
        const dateB = b.nextFollowUp ? new Date(b.nextFollowUp).getTime() : Infinity;
        return dateA - dateB;
    });
    
    const displayedFollowUps = sortedFollowUps.slice(0, MAX_DISPLAY);
    const hiddenCount = followUps.length - MAX_DISPLAY;
            ...
            <div className="space-y-3">
                ...
                {hiddenCount > 0 && (
                    <div className="text-center pt-2">
                        <Link href="/applications" className="text-xs text-amber-600 font-medium hover:underline">
                            + {hiddenCount} reminder lainnya
                        </Link>
                    </div>
                )}
            </div>
